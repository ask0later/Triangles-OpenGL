...
namespace gl {
    ...
    public:
        explicit glException(const std::string &message) : message_(std::move(message)) {}
    ...
    void SetUniform(unsigned int shader_id, const std::string &name, const glm::mat4 &value) {
        auto loc = glRUN(glGetUniformLocation, shader_id, name.data());
        glRUN(glUniformMatrix4fv, loc, 1, GL_FALSE, glm::value_ptr(value));
    }
    
    void SetUniform(unsigned int shader_id, const std::string &name, const glm::vec3 &value) {
        auto loc = glRUN(glGetUniformLocation, shader_id, name.data());
